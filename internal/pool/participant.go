...
func (pp *ParticipantPool) InjectSpike(spike *pb.SpikeEvent) error {
	...
	// If no specific targets, target all participants
	if len(targetIDs) == 0 {
		pp.mu.RLock()
		...
		pp.webrtcParticipantsMu.RLock()
		for id := range pp.webrtcParticipants {
			targetIDs = append(targetIDs, id)
		}
		pp.webrtcParticipantsMu.RUnlock()
	}

	// Inject to UDP participants
	pp.mu.RLock()
	udpCount := 0
	...
	// Inject to WebRTC participants
	pp.webrtcParticipantsMu.RLock()
	webrtcCount := 0
	for _, id := range targetIDs {
		if wp, ok := pp.webrtcParticipants[id]; ok {
			// Convert params from map[string]string to map[string]string
			params := make(map[string]string)
			for k, v := range spike.Params {
				params[k] = v
			}
			// Type assert to access AddSpike method
			if vp, ok := wp.(interface{ AddSpike(string, string, map[string]string) }); ok {
				vp.AddSpike(spike.SpikeId, spike.Type, params)
				webrtcCount++
			}
		}
	}
	...
	totalCount := udpCount + webrtcCount
	logging.LogChaos("Injected spike %s type=%s to %d participants (%d UDP, %d WebRTC)", 
		spike.SpikeId, spike.Type, totalCount, udpCount, webrtcCount)
	return nil
}
